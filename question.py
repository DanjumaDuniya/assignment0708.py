...
user_db = { "dan": { "password":1007, "balance":3000, "is_verified": False}}
...
""")
#print(display)
#command = input("Enter your login details if you have an account or you can register a new account : ").lower().strip()
...
if command == "register" or command == "login":
	...
	if command == "register":
		...
		if user_name not in user_db:
			...
			bal = float(input("enter deposit: "))
			is_verified = False
			new_user = {user_name:{"password":pin,"balance":bal,"is_verified":is_verified}}
			user_db.update(new_user)
			print("Registered Sucessfully")
			print(user_db)
			verify = input(f"Do you want to verify? yes or no \nVerification : {verification_amount} ").lower()
			if verify == "yes" and user_db[user_name]["balance"] >= verification_amount:
				cost = user_db[user_name]["balance"] - verification_amount
				user_db[user_name]["balance"] = cost
				user_db[user_name]["is_verified"] = True
		...
		if user_name in user_db and pin == user_db[user_name]["password"]:
			print("login successful")
		elif user_name in user_db and pin != user_db[user_name]["password"]:
			print(f"password mistmatch for {user_name}")
		else:
